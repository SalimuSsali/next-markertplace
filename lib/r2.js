...
/**
 * Vercel / dashboard paste issues: UTF-8 BOM, NBSP, wrapping quotes, outer spaces.
 * @param {string | undefined} raw
 * @returns {string}
 */
export function cleanR2EnvString(raw) {
  if (raw == null) return "";
  let s = String(raw)
    .replace(/^\uFEFF/, "")
    .replace(/\u00A0/g, " ")
...
export function getR2Config() {
  const accountId = cleanR2EnvString(process.env.R2_ACCOUNT_ID);
  const accessKeyId = cleanR2EnvString(process.env.R2_ACCESS_KEY_ID);
  const secretAccessKey = cleanR2EnvString(process.env.R2_SECRET_ACCESS_KEY);
  const bucket = cleanR2EnvString(process.env.R2_BUCKET_NAME);
  const publicBaseUrl = cleanR2EnvString(process.env.R2_PUBLIC_BASE_URL).replace(/\/$/, "");
