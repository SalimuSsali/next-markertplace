...
/**
 * R2 S3 host IPv4. `resolve4` is strict DNS; on some Windows networks it errors while
 * `lookup` (OS resolver) succeeds — so we try both.
 * @param {string} name
 * @returns {Promise<string>}
 */
async function resolveR2HostV4(name) {
  let first;
  try {
    const addrs = await dns.promises.resolve4(name);
    if (Array.isArray(addrs) && typeof addrs[0] === "string" && addrs[0].length > 0) {
      return addrs[0];
    }
  } catch (e) {
    first = e;
  ...
  try {
    const r = await dns.promises.lookup(name, { family: 4 });
    if (typeof r.address === "string" && r.address.length > 0) {
      return r.address;
    }
  } catch (e2) {
    const a = first != null ? errorMessageWithCause(first) : "no A record from resolve4";
    throw new Error(
      `R2 DNS: could not resolve IPv4 for ${name}: ${a} — fallback lookup: ${errorMessageWithCause(e2)}`,
    );
  }
  throw new Error(`R2 DNS: no IPv4 for ${name}`);
}
...
async function uploadViaPresignedHttpsPut(putUrl, buf, contentType) {
  ...
  const address = await resolveR2HostV4(name);
  // Connect by IPv4, but the signed request must use the real hostname in Host (not the IP) or
  // Cloudflare returns 403 to presigned PUTs.
  const port = u.port ? Number(u.port) : 443;
  const hostHeader =
    port !== 443 && Number.isFinite(port) ? `${u.hostname}:${port}` : u.hostname;
...
/**
 * Server-side upload: try SDK PutObject first (reliable with default DNS); fall back to
 * presigned PUT if needed (e.g. unusual TLS paths).
 * @param {Buffer} body
 * @param {string} contentType
 */
export async function uploadImageToR2(config, key, body, contentType) {
  ...
  try {
    ...
    const b = errorMessageWithCause(presignErr);
    throw new Error(`R2 upload failed. SDK PutObject: ${a}. Presigned PUT: ${b}`);
  }
}
